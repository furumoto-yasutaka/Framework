...
{
	pDWriteFactory->CreateTextFormat(FontList[(int)set->localeName], set->fontCollection, set->fontWeight, set->fontStyle, set->fontStretch, set->fontSize, set->localeName, &pTextFormat);
...
std::wstring DirectWrite::StringToWString(std::string oString)
{
	...
	// バッファの取得
	wchar_t* cpUCS2 = DBG_NEW wchar_t[iBufferSize];
